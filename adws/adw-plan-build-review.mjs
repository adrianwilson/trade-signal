 ...
 *
 * Usage: node adws/adw-plan-build-review.mjs <issue-number> [adw-id]
 */
...
if (!issueNumber) {
  console.error('Usage: node adws/adw-plan-build-review.mjs <issue-number> [adw-id]');
  process.exit(1);
}

const adwId = process.argv[3] || makeAdwId();
console.log(`ADW Plan+Build+Review - ID: ${adwId}, Issue: ${issueNumber}`);
...
console.log(`\nPlan+Build+Review completed for issue #${issueNumber} (ADW ID: ${adwId})`);
